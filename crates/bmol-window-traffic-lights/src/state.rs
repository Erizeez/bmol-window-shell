...
pub const PRESS_SPRING_DURATION: f32 = 0.20;
/// Perceptual bounce of the press spring (`0.0` = critically damped).
///
/// `0.30` maps to `ζ = 0.70`, which overshoots the target by ~4.6% of the
/// travel: a visible but restrained bounce that peaks around `1.188`.
pub const PRESS_SPRING_BOUNCE: f32 = 0.30;
...
mod tests {
    ...
    fn press_peaks_slightly_above_the_target_and_returns_to_rest() {
        ...
        assert!(peak <= 1.20, "peak {peak} should stay close to the intent");
        ...
        assert!(trough > 0.985, "release must not visibly undershoot: {trough}");
